...
def Forward(input, chromosome, hidden_depth, hidden_count, output_count, length):
    ...
    for i in range(0, hidden_count):
        ...
        for k in range(len(input)):
            weighted_sum += input[k] * chromosome[len(input) * i + k]
    ...
    weighted_sum = bias
    new_sum = []
    ouput_weight = len(input) * hidden_count
...
for i, chromosome in enumerate(chromosomes):
    ...
    # MSE 손실함수 계산 
    mse_err = round(np.mean((np.array(outputs) - np.array(predictions)) ** 2), 5)
    errs.append(mse_err)
